...
#include <cob_visual_servoing/cob_visual_servoing_pos.h>

#include <kdl_conversions/kdl_msg.h>


void CobVisualServoingPos::initialize()
{
	KDL::Tree my_tree;
	...
	serv_start = nh_.advertiseService("/visual_servoing/start", &CobVisualServoingPos::start_cb, this);
	serv_stop = nh_.advertiseService("/visual_servoing/stop", &CobVisualServoingPos::stop_cb, this);
	
	js_sub = nh_.subscribe("/joint_states", 1, &CobVisualServoingPos::jointstate_cb, this);
...
void CobVisualServoingPos::run()
{
	ROS_INFO("cob_visual_servoing...spinning");
	ros::spin();
}

bool CobVisualServoingPos::start_cb(cob_srvs::Trigger::Request& request, cob_srvs::Trigger::Response& response)
{
	if(b_servoing)
	{
...
bool CobVisualServoingPos::stop_cb(cob_srvs::Trigger::Request& request, cob_srvs::Trigger::Response& response)
{
	if(!b_servoing)
	{
...
void CobVisualServoingPos::jointstate_cb(const sensor_msgs::JointState::ConstPtr& msg)
{
...
bool CobVisualServoingPos::parseJointStates(std::vector<std::string> names, std::vector<double> positions, std::vector<double> velocities, KDL::JntArray& q, KDL::JntArray& q_dot)
{
...
int main(int argc, char **argv)
{
	...
	CobVisualServoingPos *cob_visual_servoing_server = new CobVisualServoingPos();
